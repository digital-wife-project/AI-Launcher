@echo off
@REM mshta vbscript:msgbox("请关闭一切杀毒软件并授予Windows命令处理程序管理员权限，即允许Windows命令处理程序具对您的设备进行更改，否则操作可能会失败！",64+4096,"证书操作")(window.close)
%1 %2
...
cls
@REM mshta vbscript:msgbox("证书导入失败，请尝试重新关闭杀毒软件并以管理员权限运行此脚本！",64+4096,"证书操作")(window.close)
exit
 
:yes
echo CertMgr Succeeded>>%temp%\your.app.name.key
@REM mshta vbscript:msgbox("证书导入成功！",64+4096,"证书操作")(window.close)
exit